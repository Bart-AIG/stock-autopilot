...
- **What it does:** (1) **coverage** — its symbols are added to the scan universe so they're analyzed every run; (2) feeds the report's **"Joint long-term port — accumulate signals (growth-on-sale)"** section. The screen = a confirmed long-term uptrend (price above a RISING 200-day MA + positive 12-1 momentum) that has **pulled back to a value entry** (≤ 50-day MA, or RSI14 ≤ 45), then **graded on a TTM fundamental value lens** (P/E, P/FCF, PEG — ✅ reasonably priced for the growth / ⚠️ rich / blank when the data tier doesn't expose `/stable/ratios-ttm`). The fundamentals are fetched **only for the candidates that pass the price gate** (a handful of names), so the extra calls are tiny, and the screen **degrades to price-only** if the endpoint is unavailable. ⚠️ **On the first live run, eyeball the P/E/P/FCF/PEG columns** — if they're all blank, the Starter tier doesn't include `ratios-ttm` and we'd need a different endpoint/source.
