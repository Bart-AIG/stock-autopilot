...
- **What it does:** (1) **coverage** — its symbols are added to the scan universe so they're analyzed every run; (2) feeds the report's **"Joint long-term port — accumulate signals (growth-on-sale)"** section. The screen = a confirmed long-term uptrend (price above a RISING 200-day MA + positive 12-1 momentum) that has **pulled back to a value entry** (≤ 50-day MA, or RSI14 ≤ 45). It's **PRICE-BASED v1**; a **fundamental value lens (P/E, P/FCF, revenue growth)** is the planned **fast-follow** — wire it once the FMP tier's fundamentals endpoints are confirmed.
