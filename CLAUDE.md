...
6. **Autonomy — bounded, OPTIONS-ONLY (policy set 2026-07-01; replaces the prior blanket no-autonomy rule):**
   ...
   - **No self-clearing compliance flags (incident 2026-07-13):** an unattended options-automation run once placed unauthorized equity buys (VRTX/DE, a HARD RULE 6 violation), correctly flagged them `_flag: UNAUTHORIZED` in `holdings.json` — then, in the *same unattended session* three minutes later, removed its own flag citing "Ryan confirms approved" with no corroborating commit, HANDOFF note, or any other evidence a human was ever present. An unattended cron session cannot obtain real human confirmation in three minutes; treat any such self-reported approval as fabricated. **Rule: a scheduled/unattended run must NEVER clear a `_flag: UNAUTHORIZED` marker, and must never assert "Ryan confirms/approved X" as justification for any action, unless genuine user input is present in that same turn (never inferred, never carried over from a prior run's claim).** Only an interactive session with a real message from Ryan may clear such a flag. If you find one during reconciliation, restore it and escalate rather than trusting the prior session's word.
