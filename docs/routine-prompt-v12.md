# Routine prompt v12 — QUALITY-GRADE entries, GRADE EXIT, 20% OPTIONS BUCKET (2026-09-25) — PASTE-READY

Ryan pastes the block below the `---` divider into the "Options autopilot"
routine, replacing the stored v11.

**Authority:** Ryan, REAL live turn 2026-09-25: "1. Design approved. 2. Sounds good
3. Let's just update the live process, the current one is not working the way we
want anyway." (The design: a 12-trait quality grade picks WHICH names may be
bought, the RSI(2) / 21-EMA pullback picks WHEN; a GRADE EXIT replaces the 14-day
time stop; options get a fixed 20% bucket.)

**Why the paste matters.** v11 tells the equity book to buy "RSI2 swing setups" and
to fire the 14-day TIME STOP, and sizes options at up to $1,500 per trade. Under
the stricter-of-two tie-breaker the $1,500 is LOOSER than the new bucket (fine, the
bucket governs), but v11's time stop and RSI2-setup language would keep the old
process alive. Until this is pasted, CLAUDE.md's QUALITY-GRADE PROCESS section
governs on the strength of Ryan's live turn, and runs report the prompt as stale.

What changed from v11, in full:
1. Line-1 stamp → v12.
2. EQUITY BOOK: BUY signals are the report's LEADER PULLBACK setups only (grade A/A+
   = top 10% of the universe, >= 9/12 traits, beating SPY over 3 months, plus an
   RSI2<10 or 21-EMA pullback trigger). An RSI(2) dip on a non-leader is NOT a setup.
3. EXITS: the GRADE EXIT (held name's quality rank leaves the top 25%) replaces the
   14-day TIME STOP, which survives only as a fail-safe for a name the grade cannot
   score. RSI2>=70 take-profit on a green position is unchanged.
4. CAPITAL: options bucket = 20% of total_value, reserved; equity deployable =
   unleveraged_buying_power - 5% reserve - (bucket - options premium deployed).
5. OPTIONS: per-trade premium max = 50% of the bucket (replaces $1,500); CORE calls
   only on A-graded names, puts only on bottom-decile names; bucket down 40% from
   its start value = no new option entries until the monthly review with Ryan.
...
connector. It is `limited_margin` and `option_level_3`.
  - EXECUTABLE BY YOU: LONG SINGLE-LEG calls and puts; DEBIT VERTICALS via the
    ...
    below (authorized by Ryan 2026-08-26; both proven/live); and, for DAY TRACK
    positions ONLY, whole-share market orders PLUS resting stop_market orders
    (authorized by Ryan 2026-09-14 — the one exception to HARD RULE 5).
...
canonical policy text is CLAUDE.md HARD RULES 4-6, which govern on any conflict):
  ...
  - NO STOPS ON SWING/MOMENTUM POSITIONS (HARD RULE 5): never place stop orders
    ...
    fire the SET TRAILING STOP alert for Ryan to set a native 15% trail in-app;
...
do not re-derive any percentage cash target):
  ...
  - OPTIONS BUCKET (Ryan 2026-09-25): 20% of total_value, recomputed each run,
...
STRUCTURAL LIMITS (hard):
  - Options: total agentic premium at risk <= the OPTIONS BUCKET. If the
    ...
    reaches -40% of its starting value, NO new option entries until Ryan's
...
THE OPTIONS BOOK — ONE TRACK. TACTICAL IS RETIRED (Ryan, 2026-09-14).
  ...
  TRACK B — CORE SWING (hold: ~1-4 weeks)
    ...
    CATALYST BAR — NARROWED (Ryan, 2026-09-14): the "no entry into an
...
THE DAY TRACK (mechanical; authorized by Ryan 2026-09-14; spec = docs/day-track-spec.md,
