# Routine prompt v11 — DAY TRACK added, TACTICAL retired, CORE catalyst bar narrowed (2026-09-14) — PASTE-READY

Ryan pastes the block below the `---` divider into the "Options autopilot"
routine, replacing the stored v10.

**Why this paste is REQUIRED (the tie-breaker inverts again).** v10 says "NO STOPS,
EVER" and hunts a TACTICAL options track. The 2026-09-14 amendments are LOOSER on
one point (the DAY TRACK may place resting stop orders) and the standing rule is
"where either is stricter, the stricter rule applies" — so an unattended run
reading v10 against the new CLAUDE.md would refuse the stops and keep hunting
TACTICAL. Until this is pasted: the DAY TRACK runs in **PAPER** mode only (needs
no orders, so no conflict), TACTICAL is retired by CLAUDE.md on the strength of
Ryan's live turn, and runs report the prompt as stale. **LIVE day trading needs
the v11 stamp on the run's own invocation.**

What changed from v10, in full:
1. Line-1 stamp → v11. TRI-BOOK.
2. SCOPE: DAY TRACK positions may carry whole-share market + resting `stop_market`
   orders (the one HARD RULE 5 exception). Swing/momentum stay no-stop.
3. TACTICAL options track REMOVED (85 evaluations, 0 trades). Options limits
   become 3 CORE open / 1 entry per run / 3 per day.
4. CORE catalyst bar NARROWED to binary events on the underlying; macro calendar
   is a sizing input, not a bar.
5. New THE DAY TRACK section (summary of `docs/day-track-spec.md`), wired into
   MANAGE, HUNT, the daily report, the weekly calibration (`graduate()` while in
   paper, `day_risk_pct` band 5–20 once live) and the Friday review.

---

```
OPTIONS + EQUITIES + DAY TRACK AUTOMATION RUN — JUDGMENT-FIRST, TRI-BOOK (prompt v11, pasted 2026-09-14)
...
THE FOUR LAWS (absolute; no thesis, no reasoning, no exception ever overrides these):
  1. Per-position premium at risk: max $1500 for options, and $1500 is
     reserved for A++ CORE SWING setups only (TACTICAL is RETIRED 2026-09-14);
...
STRUCTURAL LIMITS (hard):
  - Options: max 3 open agentic CORE positions (hedge excluded; a legged
